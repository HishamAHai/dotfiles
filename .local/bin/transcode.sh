...
##
    ...
    /usr/local/bin/ffmpeg -hide_banner -loglevel quiet -stats -i $i -map 0:v -map 0:a -c:v dnxhd -vf "scale=1920:1080,format=yuv422p" -b:v 120M -c:a pcm_s16le transcoded/$output.mov
    done
    ##
    ...
    /usr/local/bin/ffmpeg -hide_banner -loglevel quiet -stats -y -hwaccel cuda -hwaccel_output_format cuda -i $1 -c:a aac -c:v hevc_nvenc -b:v 8M -pix_fmt yuv420p -fps_mode passthrough $(dirname $1)/../finalized/$file.mp4
fi
